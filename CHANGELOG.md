...
* balena-*-block: Source includes from / as well as inside device repositories [Alex Gonzalez]
* balena-build-block: Allow to specify the deploy dir [Alex Gonzalez]
* balena-lib: Use balena.yml if present [Alex Gonzalez]
* balena-deploy-block.sh: Deploy using compose files [Alex Gonzalez]
* balena-deploy: Deploy using compose files [Alex Gonzalez]
* balena-docker: Move compose file creation function from balena-deploy.inc [Alex Gonzalez]
* balena-lib: balena_lib_release: Use local cache if available [Alex Gonzalez]
* balena-lib: get_label_from_image: Return nothing if not found [Alex Gonzalez]
