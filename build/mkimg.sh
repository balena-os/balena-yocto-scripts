...
populate_image() {
	...
	while [ "${_partitionIndex}" -lt "${_partitionCount}" ]; do
		...
		if [ -n "${_filesetImages}" ]; then
			...
			read -r DOCKER_HOST DOCKER_PIDFILE <<< "$(balena_docker_start "${_tmpDockerDir}/data" "${_tmpDockerDir}")"
			export DOCKER_HOST
		...
		if [ -n "${_serviceImages}" ] || [ -n "${_overlayImages}" ] || [ -n "${_bootableServiceImages}" ]; then
			...
			read -r DOCKER_HOST DOCKER_PIDFILE <<< "$(balena_docker_start "${_data_dir}" "${_tmpDir}")"
			export DOCKER_HOST
			export DOCKER_PIDFILE
			balena_docker_wait "${DOCKER_HOST}"
